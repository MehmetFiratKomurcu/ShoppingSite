...
if ($_SESSION['admin'] == 0) {
  ...
  $sql = "SELECT messageContent,datetime,Topic FROM mesajlar WHERE sender = '$tempuser'";
  $result =mysqli_query($conn,$sql);
}
else if ($_SESSION['admin'] == 1) {
  $tempuser = $_SESSION['username'];
  $sql = "SELECT messageContent,datetime,Topic,sender FROM mesajlar WHERE answer != 'NULL'";
  $result =mysqli_query($conn,$sql);
}
?>
<div class="sentContainer">
        ...
        if ($_SESSION['admin'] == 0) {
          if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result) ) {
              ...
              "</td><td style='display:none'>".$row['messageContent']."</td></tr>";
            }
          }
          else {
          ...
          if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result) ) {
              echo "<tr><td>".$row['Topic']."</td><td>".$row['datetime'].
              "</td><td style='display:none'>".$row['messageContent']."</td>
              <td>".$row['sender']."</td></tr>";
            }
          }
          else {
  ...
  <div class="secondContainer" style="display:none;" id="msgContnt">
    ...
    <label class="secondLabel contentLabel"><b><br>Content: </b></label>
    <textarea id="txtarea"value ="" readonly class="secondTextarea"></textarea>
    <span id="editbtn" class="FormBtn"><br>Back</span>
  </div>
</div>
<script>
